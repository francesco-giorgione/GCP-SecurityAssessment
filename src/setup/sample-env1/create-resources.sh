...
BUCKET_NAMES=("bucket-free-1-unique123456" "bucket-free-2-unique123456")
FIRESTORE_ENABLED=0
...
for VM in "${VM_NAMES[@]}"; do
  if gcloud compute instances describe "$VM" --zone="$ZONE" &> /dev/null; then
    ...
    # Add --can-ip-forward only for vm-free-1
    if [[ "$VM" == "vm-free-1" ]]; then
  ...
  if gcloud sql instances describe "$SQL_INSTANCE_NAME" &> /dev/null; then
    ...
    gcloud sql instances create "$SQL_INSTANCE_NAME" \
      ...
      --region="$REGION" \
      --storage-type=SSD \
      --database-flags=local_infile=on \
      --storage-size=10GB \
      --quiet
  fi
